...
import { SystemModule } from '../../types/role'
...
const RoleManagement = lazy(() => import('./roles/RoleManagement').then(m => ({ default: m.RoleManagement })))
...
const SimpleAdminDashboard: React.FC = () => {
  ...
  const { can, hasAny, currentRole, isSuperAdmin } = useRolePermissions()

  const viewModuleMap: Partial<Record<ActiveView, SystemModule>> = {
    'invoices': 'invoices',
    'reporting-hub': 'reporting',
    'reporting-invoices': 'reporting',
    'payments': 'payments',
    'quotes': 'quotes',
    'reporting-quotes': 'reporting',
    'contracts': 'contracts',
    ...
    'reporting-hr-performance': 'reporting',
    'hr-compensation': 'compensation',
    'reporting-hr-compensation': 'reporting',
    ...
    'leads': 'leads',
    'reporting-leads': 'reporting',
    'opportunities': 'opportunities',
    'reporting-opportunities': 'reporting',
    'products': 'invoices',
  }
  ...
  const renderMainContent = () => {
    const requiredModule = viewModuleMap[activeView];
    if (requiredModule && !can(requiredModule, 'view') && !isSuperAdmin) {
      return (
        <div className="p-8 max-w-2xl mx-auto my-12 bg-white rounded-3xl border border-rose-100 shadow-xl text-center space-y-4">
          ...
          <p className="text-sm text-gray-600">
            Your assigned role (<span className="font-bold text-indigo-700">{currentRole?.name || 'Assigned Role'}</span>) does not have permission to access the <strong className="text-gray-900 capitalize">{activeView.replace(/-/g, ' ')}</strong> module.
          </p>
          <p className="text-xs text-gray-500">
            If you need access to this section, please contact your Super Administrator (admin@kdadks.com) to update your permissions.
          </p>
          <div className="pt-2 flex justify-center gap-3">
            <button
              onClick={() => navigate('/admin')}
              className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-200"
            >
              Return to Dashboard
            </button>
          </div>
        ...
        <nav className="flex-1 py-4 overflow-y-auto min-h-0">
          <ul className="space-y-1 px-2">
            ...
            <li>
              ...
              {/* Section: Sales */}
            {(can('leads', 'view') || can('opportunities', 'view') || can('quotes', 'view') || can('contracts', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
                ...
                {(openSections.sales || !sidebarOpen) && (
                  <>
                    ...
                    {/* Opportunities */}
                    {(can('opportunities', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Contracts */}
                    {(can('contracts', 'view') || isSuperAdmin) && (
                      <li>
                        <button
            ...
            {/* Section: Customers */}
            {(can('customers', 'view') || can('customer_360', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
                ...
                {(openSections.customers || !sidebarOpen) && (
                  <>
                    {/* Customers */}
                    {(can('customers', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Customer 360 Hub */}
                    {(can('customer_360', 'view') || isSuperAdmin) && (
                      <li>
                        <button
            ...
            {(can('invoices', 'view') || can('rate_cards', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
                ...
                {(openSections.catalog || !sidebarOpen) && (
                  <>
                    {/* Products */}
                    {(can('invoices', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Rate Cards */}
                    {(can('rate_cards', 'view') || isSuperAdmin) && (
                      <li>
                        <button
            ...
            {(can('subscriptions', 'view') || can('invoices', 'view') || can('payments', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
                ...
                {(openSections.billing || !sidebarOpen) && (
                  <>
                    {/* Subscriptions */}
                    {(can('subscriptions', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Payments */}
                    {(can('payments', 'view') || isSuperAdmin) && (
                      <li>
                        <button
            ...
            {/* Section: Finance */}
            {(can('finance', 'view') || can('income', 'view') || can('expenses', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
                ...
                {(openSections.finance || !sidebarOpen) && (
                  <>
                    ...
                    {/* Expenses */}
                    {(can('expenses', 'view') || isSuperAdmin) && (
                      <li>
                        <button
            ...
            {(can('employees', 'view') || can('attendance', 'view') || can('leave', 'view') || can('compensation', 'view') || can('performance', 'view') || can('settlement', 'view') || can('policies', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
                ...
                {(openSections.hr || !sidebarOpen) && (
                  <>
                    {/* Employees & Docs */}
                    {(can('employees', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Attendance */}
                    {(can('attendance', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Compensation */}
                    {(can('compensation', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Reviews & Feedback */}
                    {(can('performance', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Settlement */}
                    {(can('settlement', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Policies & SOPs */}
                    {(can('policies', 'view') || isSuperAdmin) && (
                      <li>
                        <button
            ...
            {(can('announcements', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
            ...
            {/* Section: Governance */}
            {(can('board_resolutions', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
            ...
            {(can('reporting', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
            ...
            {(can('settings', 'view') || can('roles', 'view') || isSuperAdmin) && (
              <>
                <li className="pt-3">
                  {sidebarOpen ? (
                    <button
                ...
                {(openSections.configuration || !sidebarOpen) && (
                  <>
                    {/* Settings */}
                    {(can('settings', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                    ...
                    {/* Roles & Permissions */}
                    {(can('roles', 'view') || isSuperAdmin) && (
                      <li>
                        <button
                          onClick={() => navigate('/admin/roles')}
                          title={!sidebarOpen ? 'Roles & Permissions' : undefined}
                          className={`w-full flex items-center ${!sidebarOpen ? 'justify-center' : ''} px-3 py-2 text-sm font-medium rounded-md transition-colors ${
                            activeView === 'roles' || activeView === 'users'
                              ? 'bg-indigo-100 text-indigo-700 font-semibold'
                              : 'text-gray-700 hover:bg-gray-100'
                          }`}
                        >
                          <ShieldCheck className="w-5 h-5 flex-shrink-0 text-indigo-600" />
                          {sidebarOpen && <span className="ml-3">Roles & Permissions</span>}
                        </button>
                      </li>
                    )}
                  </>
            ...
            <div className="mb-3">
              ...
              <div className="mt-1">
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200">
                  {currentRole?.name || (isSuperAdmin ? 'Super Admin' : 'Standard User')}
                </span>
              </div>
            </div>
          )}
        ...
        <main className="p-6">
          ...
          {activeView === 'dashboard' ? (
            <div>
              ...
              {(can('leads', 'view') || can('quotes', 'view') || can('customer_360', 'view') || can('contracts', 'view') || isSuperAdmin) && (
                <div className="mb-6">
              ...
              {(can('invoices', 'view') || can('payments', 'view') || can('subscriptions', 'view') || can('finance', 'view') || isSuperAdmin) && (
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    {/* Revenue Card */}
                    {(can('finance', 'view') || can('invoices', 'view') || isSuperAdmin) && (
                      <div 
                        className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 cursor-pointer hover:shadow-md transition-shadow"
                        onClick={() => navigate('/admin/finance')}
                      >
              ...
              {(can('employees', 'view') || can('compensation', 'view') || can('settlement', 'view') || isSuperAdmin) && (
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    {(can('settlement', 'view') || isSuperAdmin) && (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm text-gray-600">F&F Settlements</p>
                            <p className="text-2xl font-semibold text-gray-900 mt-1">
                              {statsLoading ? '...' : dashboardStats.settlements}
                            </p>
                            <p className="text-xs text-red-600 mt-1">Exit Calculations</p>
                          </div>
              ...
              <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
                  ...
                  {/* Create Invoice */}
                  {(can('invoices', 'create') || can('invoices', 'view') || isSuperAdmin) && (
                    <button
                      onClick={() => navigate('/admin/invoices')}
                      className="flex flex-col items-center p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors"
                    >
                  ...
                  {/* Create Quote */}
                  {(can('quotes', 'create') || can('quotes', 'view') || isSuperAdmin) && (
                    <button
                      onClick={() => navigate('/admin/quotes')}
                  ...
                  {/* Create Contract */}
                  {(can('contracts', 'create') || can('contracts', 'view') || isSuperAdmin) && (
                    <button
                      onClick={() => navigate('/admin/contracts')}
                  ...
                  {/* Add Employee */}
                  {(can('employees', 'create') || can('employees', 'view') || isSuperAdmin) && (
                    <button
                      onClick={() => navigate('/admin/hr/employees')}
